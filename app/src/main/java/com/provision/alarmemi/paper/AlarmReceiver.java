...
public class AlarmReceiver extends BroadcastReceiver {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (!alarm.daysOfWeek.isRepeatSet()) {
			...
			if (alarm.cloudEnabled) {
				...
				editor.putString("need_remove",
						prefs.getString("need_remove", "") + "||"
								+ alarm.cloudName + "|" + alarm.cloudKey);
				editor.commit();
				ConnReceiver.RemoveNotRemoved(context);
				Alarms.deleteAlarm(context, alarm.id);
			} else Alarms.enableAlarm(context, alarm.id, false);
		} else {
	...
	private void updateNotification(Context context, Alarm alarm, int timeout) {
		...
		Intent viewAlarm = new Intent(context, SetAlarmFragment.class);
